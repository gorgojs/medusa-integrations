...
```mdx
<CardList items={[
  { title: "Основные концепции", href: "/medusa-modules/integration/concepts" },
  { title: "Чтение параметров", href: "/medusa-modules/integration/reading-options" }
]}/>
```

Only `title` and `href`. Used for a module's `## Дальнейшие шаги`/`## Next steps` and a plugin's `## Разработчикам`/`## Development` section. Not used for the `## Материалы`/`## References` closing section of a module how-to page — that's a plain markdown bullet list of links instead (see `module-docs-style.md`).
