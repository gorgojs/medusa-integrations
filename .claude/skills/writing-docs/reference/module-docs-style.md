...
This applies to fenced code examples (` ```ts `, plain-literal fences, terminal commands). It does not apply to `<MedusaTypeList>` — that's a reference table, not a worked example, and it follows its own rules in `mdx-components.md` (a lead-in sentence naming what the table covers, a grouping bullet list above it when rows apply under different conditions and there are more than ~6). Don't force a "follow-up paragraph" onto a table.
...
The **follow-up** explains what the code doesn't show on its face: what comes back, its type, what has already been done to it, which case it doesn't cover. Anchor it to something literally in the block — an identifier, a return type, a token that's actually there. If the paragraph would read exactly the same with the code block deleted, it's not a follow-up, it's a preview or a transition; rewrite it around what's on screen.
...
Never narrate the code line by line — narration is not a follow-up, it's noise.

Every code block gets both a lead-in and a follow-up. No exceptions:

- **A skeleton immediately followed by the implementation that fills it in** still gets its own follow-up after the first block, grounded in what's already on screen (a base class it extends, a type it's missing) — not a "the next section adds X" preview, which is a transition wearing a follow-up's clothes.
- **A block already followed by a `<Note>`** still gets a follow-up paragraph first; the `<Note>` carries an additional caveat, not the base explanation. Don't let the `<Note>` substitute for it.
- **A pure literal** — an `.env` snippet, a terminal command, a registration entry in `medusa-config.ts` — gets a follow-up too: what the reader does with it next, or what happens once it's in place.
...
- <CardList> of items, one per linked page
```
