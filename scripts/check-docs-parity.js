#!/usr/bin/env node
/**
 * Structural parity gate for bilingual doc pages. Zero dependencies.
 *
 * `en.mdx` and `ru.mdx` in the same directory must have the same structure. What must match, and
 * what legitimately differs, is specified in `writing-docs/reference/bilingual-parity.md` in the
 * `gorgo` plugin; this script is that specification made executable, replacing a list of `diff`
 * one-liners a human had to remember to run.
 *
...
function topicDirs(root) {
  ...
  const walk = dir => {
    ...
    if (entries.some(e => e.isFile() && /^(en|ru)\.mdx$/.test(e.name))) out.push(dir)
...
function shape(src) {
  ...
  return {
    ...
    headings: outside.split('\n')
      .map(l => l.match(/^(#{1,6})\s+\S/))
      .filter(Boolean)
      .map(m => m[1].length),
    fences: fences.map(normalizeFence),
    // MedusaTypeList field names, in order
    fieldNames: [...body.matchAll(/^\s*name:\s*"?([\w.[\]-]+)"?/gm)].map(m => m[1]),
    flags: [...body.matchAll(/\b(optional|defaultValue):\s*("[^"]*"|\S+)/g)]
      .map(m => normalizeFlag(m[1], m[2])),
    hrefs: [...body.matchAll(/\]\((\/[^)\s#]*)(?:#[^)\s]*)?\)/g)].map(m => normalizeHref(m[1])),
  }
}
...
function compare(dir) {
  ...
  if (!fs.existsSync(en) || !fs.existsSync(ru)) {
    problems.push({
      topic: rel, check: 'missing-language',
