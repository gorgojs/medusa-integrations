...
function getProcessedCount() {
  ...
  return readdirSync(CHANGESET_DIR).filter((f) => /^auto-\d+-.+\.md$/.test(f))
    .length;
}
...
async function resolveAuthors(commits) {
  ...
  try {
    return await resolveCommitAuthors(
...
async function main() {
  ...
  console.log(
    `Base: ${baseRef ?? '(all history)'}, commits: ${commits.length}, existing: ${index}`,
  );
