...
Add the configurations of the Integration Module provider and of the Medusa Payment Module in your `medusa-config.js` file of the Medusa Admin application:
  ...
  types={[
    {
      name: "id",
      type: "string",
      description: "Suffix in the resulting integration key `int_<identifier>_<id>`; when registering multiple instances of the same class, makes each one unique.",
      optional: false,
      expandable: false,
      children: []
    },
...
<Note type="warning">For the payment provider to use the integration's settings, pass the integration's `id` in the payment provider's options.</Note>
