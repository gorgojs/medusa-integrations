---
title: "Getting Started with T-Kassa Payment for Medusa (legacy v1)"
...
This guide covers configuring the legacy 1.x version, where you set the plugin options in `medusa-config.js`. The current version 2 keeps these settings in the [Integration Module](/medusa-modules/integration), so a store admin edits them in Admin without changing configuration files or redeploying. For a new installation, follow [Getting Started with T-Kassa Payment for Medusa](/medusa-integrations/t-kassa/getting-started).
